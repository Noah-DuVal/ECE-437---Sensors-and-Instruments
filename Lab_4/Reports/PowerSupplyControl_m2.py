...
# check that it is actually the power supply
    try:
        ...
        if (device_temp.query("*IDN?") == 'Agilent Technologies,34461A,MY53207929,A.01.10-02.25-01.10-00.35-01-01\n'):
            digital_multimeter_id = i 
        if (device_temp.query("*IDN?") == 'Keysight Technologies,34461A,MY53212931,A.02.08-02.37-02.08-00.49-01-01\n'):
            digital_multimeter_id = i            
        if (device_temp.query("*IDN?") == 'KEYSIGHT TECHNOLOGIES,MSO-X 3024T,MY55100339,07.50.2021102830\n'):
            oscilloscope_id = i                        
        device_temp.close()
    except:
...
# then the applied voltage will not be the same as the measured voltage.
    ...
    for v in output_voltage:
        ...
        measured_voltage_tmp = oscilloscope.query("MEASure:VRMS? DISPlay,DC,CHANnel2")
